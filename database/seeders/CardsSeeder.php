...
class CardsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->storeCards($records, $attributes, $races);

        // some missing GOAT format cards
        $json = Storage::disk('local')->get('missing-goat-cards.json');
        $records = array_values(json_decode($json, true));

        $this->storeCards($records, $attributes, $races);
    }

    private function storeCards(array $records, array $attributes, array $races): void
    {
            ...
            foreach ($records as $cards) {
                foreach ($cards as $card) {
                    ...
                    $newCard = Card::create([
                        ...
                        'race_id' => (in_array(strtolower($card['race']), array_keys($races))) 
                                        ? $races[strtolower($card['race'])] : null,
